...
class TelegramBot:
    ...
    def format_update_result(self, result):
        if isinstance(result, tuple) and len(result) == 3:
            ...
            if key == "update_success":
                cleanup_str = (
                    self.t("cleanup_removed") if params.get("cleaned_up") else ""
                )
                template_key = (
                    "update_success_running"
                    if params.get("was_running")
                    else "update_success_stopped"
                )
                return self.t(
                    template_key, name=params.get("name", ""), cleanup=cleanup_str
                )
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    self.t("settings_btn"), callback_data="menu_main_settings"
                )
            ]
        ]
    ...
    def _get_settings_main_ui(self):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    self.t("btn_interval"), callback_data="menu_set_interval"
                )
            ],
            [InlineKeyboardButton(self.t("btn_delay"), callback_data="menu_set_delay")],
            [
                InlineKeyboardButton(
                    self.t("btn_cleanup"), callback_data="menu_set_cleanup"
                )
            ],
            [
                InlineKeyboardButton(
                    self.t("btn_stopped"), callback_data="menu_set_stopped"
                )
            ],
            [
                InlineKeyboardButton(
                    self.t("btn_quarantine"), callback_data="menu_set_quarantine"
                )
            ],
        ]
        return msg, InlineKeyboardMarkup(keyboard)
    ...
    def _get_quarantine_ui(self):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    f"{'✅ ' if days == 0 else ''}{self.t('day_0')}",
                    callback_data="set_quarantine_0",
                ),
                InlineKeyboardButton(
                    f"{'✅ ' if days == 1 else ''}{self.t('day_1')}",
                    callback_data="set_quarantine_1",
                ),
                InlineKeyboardButton(
                    f"{'✅ ' if days == 3 else ''}{self.t('days_3')}",
                    callback_data="set_quarantine_3",
                ),
            ],
            [
                InlineKeyboardButton(
                    f"{'✅ ' if days == 5 else ''}{self.t('days_5')}",
                    callback_data="set_quarantine_5",
                ),
                InlineKeyboardButton(
                    f"{'✅ ' if days == 7 else ''}{self.t('days_7')}",
                    callback_data="set_quarantine_7",
                ),
            ],
            [
                InlineKeyboardButton(
                    self.t("btn_quarantine_list"), callback_data="menu_quarantine_list"
                )
            ],
            [
                InlineKeyboardButton(
                    self.t("btn_back"), callback_data="menu_main_settings"
                )
            ],
        ]
        return msg, InlineKeyboardMarkup(keyboard)
    ...
    def _get_quarantine_list_ui(self):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    self.t("btn_back"), callback_data="menu_set_quarantine"
                )
            ]
        ]
        return msg, InlineKeyboardMarkup(keyboard)
    ...
    def _get_interval_ui(self):
        ...
        keyboard = [
            [
                ...
                InlineKeyboardButton(
                    f"{'✅ ' if days == 7 else ''}{self.t('days_7')}",
                    callback_data="set_interval_7",
                ),
                InlineKeyboardButton(
                    f"{'✅ ' if days == 30 else ''}{self.t('days_30')}",
    ...
    def _get_delay_ui(self):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    f"{'✅ ' if delay == 0 else ''}0s", callback_data="set_delay_0"
                ),
                InlineKeyboardButton(
                    f"{'✅ ' if delay == 2 else ''}2s", callback_data="set_delay_2"
                ),
                InlineKeyboardButton(
                    f"{'✅ ' if delay == 5 else ''}5s", callback_data="set_delay_5"
                ),
            ],
            [
                InlineKeyboardButton(
    ...
    def _get_cleanup_ui(self):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    f"{'✅ ' if enabled else ''}{self.t('enabled')}",
                    callback_data="set_cleanup_1",
                ),
                InlineKeyboardButton(
                    f"{'✅ ' if not enabled else ''}{self.t('disabled')}",
                    callback_data="set_cleanup_0",
                ),
            ],
            [
                InlineKeyboardButton(
    ...
    def _get_stopped_ui(self):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    f"{'✅ ' if enabled else ''}{self.t('enabled')}",
                    callback_data="set_stopped_1",
                ),
                InlineKeyboardButton(
                    f"{'✅ ' if not enabled else ''}{self.t('disabled')}",
                    callback_data="set_stopped_0",
                ),
            ],
            [
                InlineKeyboardButton(
    ...
    async def send_update_notification(
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    self.t("update_btn"), callback_data=f"update_{container_name}"
                ),
                InlineKeyboardButton(
                    self.t("ignore_btn"), callback_data=f"ignore_{container_name}"
                ),
            ],
            [
                InlineKeyboardButton(
                    self.t("auto_update_btn"), callback_data=f"auto_{container_name}"
                )
            ],
        ]
    ...
    async def button_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if action == "ignore":
            ...
            if not remote_digest:
                ...
                containers = await asyncio.to_thread(
                    self.docker_manager.get_containers, include_stopped
                )
        ...
        if action in ["update", "auto"]:
            if action == "auto":
                ...
                await query.edit_message_text(
                    text=self.t("auto_updating_msg", container=container_name)
                )
            else:
                await query.edit_message_text(
                    text=self.t("updating_msg", container=container_name)
                )
            ...
            if not target:
                await query.edit_message_text(
                    text=self.t("not_found_msg", container=container_name)
                )
                return
            ...
            result = await asyncio.to_thread(
                self.docker_manager.update_container, target.id, cleanup
            )
            success = result[0]
        ...
        for i, container in enumerate(containers):
            ...
            if new_image and remote_digest:
                ...
                if ignored_digest == remote_digest:
                    logger.info(
                        f"Update for {container.name} was previously ignored. Skipping."
                    )
                    continue
                ...
                q_days = (
                    int(q_label)
                    if q_label and q_label.isdigit()
                    else self.config_db.get_quarantine_days()
                )
                ...
                if q_days > 0:
                    ...
                    remote_created_iso = (
                        remote_info.get("created") if remote_info else None
                    )
                    if remote_created_iso:
                        try:
                            ...
                            logger.error(
                                f"Failed to parse remote creation date {remote_created_iso}: {e}"
                            )
                    ...
                    if not q_record:
                        ...
                        if is_in_quarantine:
                            logger.info(
                                f"Quarantine tracking started for {container.name} (age: {age_days}d, digest: {remote_digest})"
                            )
                            self.config_db.update_quarantine_record(
                                container.name, remote_digest, 0, remote_created_iso
                            )
                            continue
                        else:
                            # Image is already old enough! No need to quarantine.
                            logger.info(
                                f"Container {container.name} update is {age_days}d old, bypassing {q_days}d quarantine."
                            )
                    else:
                        if q_record["detected_digest"] == remote_digest:
                            ...
                            if is_in_quarantine:
                                logger.info(
                                    f"Container {container.name} is in quarantine for {(q_days - age_days)} more days. Skipping."
                                )
                                continue
                            else:
                                logger.info(
                                    f"Container {container.name} passed {q_days} days quarantine."
                                )
                ...
                label_auto = (
                    str(labels.get("telegramtower.autoupdate", "")).lower() == "true"
                )
                ...
                if label_auto or db_auto:
                    ...
                    cleanup = self.config_db.get_cleanup_old_image()
                    result = await asyncio.to_thread(
                        self.docker_manager.update_container, container.id, cleanup
                    )
                    success = result[0]
                    msg_str = self.format_update_result(result)
