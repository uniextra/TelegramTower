...
class RegistryFetcher:
    ...
    def parse_image_name(image_name):
        ...
        elif len(parts) == 2 and ('.' not in parts[0] and ':' not in parts[0] and parts[0] != 'localhost'):
        ...
        try:
            ...
            if resp.status_code == 401 and 'Www-Authenticate' in resp.headers:
                ...
                if token_resp.status_code == 200:
                    return token_resp.json().get('token') or token_resp.json().get('access_token')
    ...
    def fetch_manifest(registry, repository, reference, token):
        ...
        if resp.status_code != 200:
            logger.error(f"Failed to fetch manifest for {repository}:{reference} - Status: {resp.status_code}")
            return None
        ...
        if 'manifest.list' in media_type or 'image.index' in media_type:
            ...
            for m in manifests:
                ...
                if platform.get('architecture') in ['amd64', 'arm64'] and platform.get('os') == 'linux':
                    ...
                    return RegistryFetcher.fetch_manifest(registry, repository, m.get('digest'), token)
            
            # Fallback to first if linux not found
            if manifests:
                return RegistryFetcher.fetch_manifest(registry, repository, manifests[0].get('digest'), token)
